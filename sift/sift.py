    ...
    def register_sparse_param(self):
        ...
        for name, layer in self.model.named_parameters():
            ...
            if any([module in name for module in self.sparse_module]):
                ...
                train_num = min(int(self.sparse_rate * layer.numel()) + 1, layer.numel())
                ...
                #sparse_idx = torch.flatten(abs(layer.data)).topk(train_num).indices
                ## Random pick
                sparse_idx = torch.tensor(random.sample(list(range(layer.numel())), train_num), dtype=torch.int)
            ...
            with torch.no_grad():
                ...
                for name, layer in self.named_trainable_parameters():
                    ...
                    if self.grad_acc_count[name] == self.grad_acc:
                        ...
                        delta = layer.data + torch.sparse_coo_tensor(sparse_param.idx, sparse_param, layer.shape).to(layer)
